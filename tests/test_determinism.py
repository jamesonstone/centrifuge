...
2,Bob,200.00,2024-01-16"""
        ...
        for i, content in enumerate([csv1, csv2]):
            ...
            # Sort by ID for comparison (use title case after normalization)
            cleaned_df = cleaned_df.sort_values('Id').reset_index(drop=True)
            # Drop row_number as it depends on original order
            cleaned_df = cleaned_df.drop('row_number', axis=1)
            results.append(cleaned_df)

        # Results should be identical when sorted (excluding row_number)
    ...
    def _create_simple_schema(self) -> Schema:
        ...
        return Schema(
            ...
            columns=[
                ColumnDefinition(
                    name="Id",  # Matches normalized header
                    display_name="ID",
                ...
                ColumnDefinition(
                    name="Name",  # Matches normalized header
                    display_name="Name",
                ...
                ColumnDefinition(
                    name="Amount",  # Matches normalized header
                    display_name="Amount",
                ...
                ColumnDefinition(
                    name="Date",  # Matches normalized header
                    display_name="Date",
...
class TestPropertyBased:
    ...
    def test_normalization_properties(self, department, amount, date_str):
        ...
        # Create test data (use title case for normalized headers)
        df = pd.DataFrame([{
            'row_uuid': str(uuid4()),
            'row_number': 1,
            'Department': department,  # Title case after normalization
            'Amount': amount,
            'Date': date_str
        }])
        ...
        schema = Schema(
            ...
            columns=[
                ColumnDefinition(
                    name="Department",  # Matches normalized header
        ...
        # 2. Amount is always numeric string with proper formatting
        amount_val = cleaned_df.iloc[0]['Amount']
        assert isinstance(amount_val, str)
        # Should be a properly formatted decimal string
        try:
            float_val = float(amount_val)
            assert float_val >= 0
        except ValueError:
            assert False, f"Amount should be convertible to float: {amount_val}"
    ...
    def test_debit_credit_invariant(self, text, has_debit, has_credit, debit_amount, credit_amount):
        ...
        df = pd.DataFrame([{
            ...
            'Debit Amount': debit_amount if has_debit else 0,  # Title case with space
            'Credit Amount': credit_amount if has_credit else 0  # Title case with space
        }])
        ...
        schema = Schema(
            ...
            columns=[
                ...
                ColumnDefinition(
                    name="Debit Amount",  # Title case with space
                ...
                ColumnDefinition(
                    name="Credit Amount",  # Title case with space
            ...
            constraints=SchemaConstraints(debit_xor_credit=True)  # Enable the XOR constraint
        )

        validator = Validator(schema)
        valid_df, quarantine_df, errors = validator.validate(df)

        # Check the invariant - XOR means exactly one must be positive
        debit_positive = debit_amount > 0 if has_debit else False
        credit_positive = credit_amount > 0 if has_credit else False

        if debit_positive != credit_positive:  # XOR condition: exactly one is positive
            # Valid case
            assert len(valid_df) == 1
            assert len(quarantine_df) == 0
        else:
            # Invalid case - both positive, both zero, or both negative
    ...
    def test_reference_number_normalization(self, reference):
        ...
        df = pd.DataFrame([{
            'row_uuid': str(uuid4()),
            'row_number': 1,
            'Reference Number': reference  # Title case with space
        }])
        ...
        schema = Schema(
            ...
            columns=[
                ColumnDefinition(
                    name="Reference Number",  # Title case with space
        ...
        result = cleaned_df.iloc[0]['Reference Number']
