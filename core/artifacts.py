...
class ArtifactManager:
    ...
    def _generate_metrics_json(self, metrics: Metrics) -> Dict[str, Any]:
        ...
        metrics_dict = {
            ...
            'llm_calls_made': metrics.llm_calls_count if metrics.llm_calls_count and metrics.llm_calls_count > 0 else None,
            'llm_fixed_count': metrics.llm_fixed_count,
            'llm_fixed_percentage': llm_fixed_percentage,
            'llm_tokens_used': metrics.llm_tokens_used if metrics.llm_tokens_used and metrics.llm_tokens_used > 0 else None,
            'llm_cost_estimate': float(metrics.llm_cost_estimate) if metrics.llm_cost_estimate and float(metrics.llm_cost_estimate) > 0 else None,
