...
from core.models import (
    RunManifest, RunMetrics, AuditEvent, Diff,
...
class ArtifactManager:
    ...
    def _generate_diff_csv(self, diffs: List[Diff]) -> Dict[str, Any]:
    ...
    def _generate_manifest_json(self, manifest: RunManifest) -> Dict[str, Any]:
    ...
    def _generate_metrics_json(self, metrics: RunMetrics) -> Dict[str, Any]:
        ...
        metrics_dict = {
            ...
            'rules_fixed_percentage': metrics.rules_fixed_percentage,
            
            'llm_calls_made': metrics.llm_calls_made,
            'llm_fixed_count': metrics.llm_fixed_count,
            'llm_fixed_percentage': metrics.llm_fixed_percentage,
            ...
            'errors_by_column': metrics.errors_by_column,
            
            'edit_caps': metrics.edit_caps,
    ...
    def _generate_summary_md(
        ...
        lines = [
            ...
            f"- **Clean Rows:** {metrics.clean_rows:,} ({metrics.success_rate:.1%})",
            f"- **Quarantined Rows:** {metrics.quarantined_rows:,} ({metrics.quarantine_rate:.1%})",
            "",
            "## Data Quality Improvements",
            f"### Rules Engine",
            f"- **Changes Applied:** {len(rules_changes):,}",
            f"- **Rows Fixed:** {metrics.rules_fixed_count:,}",
            f"- **Fix Rate:** {metrics.rules_fixed_percentage:.1%}",
            "",
            f"### LLM Processing",
            f"- **API Calls:** {metrics.llm_calls_made:,}",
            f"- **Changes Applied:** {len(llm_changes):,}",
            f"- **Rows Fixed:** {metrics.llm_fixed_count:,}",
            f"- **Fix Rate:** {metrics.llm_fixed_percentage:.1%}",
            f"- **Tokens Used:** {metrics.llm_tokens_used:,}" if metrics.llm_tokens_used else "- **Tokens Used:** N/A",
            f"- **Estimated Cost:** ${metrics.llm_cost_estimate:.2f}" if metrics.llm_cost_estimate else "- **Estimated Cost:** N/A",
            "",
            f"### Cache Performance",
        ...
        if quarantine_stats and quarantine_stats.get('errors_by_category'):
            ...
            for category, count in quarantine_stats['errors_by_category'].items():
                percentage = count / metrics.quarantined_rows * 100 if metrics.quarantined_rows > 0 else 0
        ...
        if metrics.phase_timings:
            ...
            lines.append(f"- **Total Duration:** {metrics.total_duration_seconds:.2f}s")
            lines.append("")
        
        # Column-specific issues
        if metrics.errors_by_column:
            lines.append("### Issues by Column")
            sorted_columns = sorted(
                metrics.errors_by_column.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5]
            for column, count in sorted_columns:
                lines.append(f"- **{column}:** {count} errors")
            lines.append("")
        
        # Edit caps
        if metrics.edit_caps:
            lines.append("### Edit Cap Usage")
            for column, cap_info in metrics.edit_caps.items():
                usage = cap_info.get('used', 0) / cap_info.get('limit', 1) * 100
                lines.append(f"- **{column}:** {cap_info.get('used', 0)}/{cap_info.get('limit', 0)} ({usage:.1f}%)")
            lines.append("")
        
        # Confidence distribution
        if metrics.confidence_distribution:
            lines.append("### LLM Confidence Distribution")
            for bucket, count in sorted(metrics.confidence_distribution.items()):
                lines.append(f"- **{bucket}:** {count} mappings")
            lines.append("")
        
        # Summary
        lines.extend([
            "## Summary",
            f"The pipeline successfully processed {metrics.success_rate:.1%} of input rows. "
            f"Rules-based cleaning fixed {metrics.rules_fixed_percentage:.1%} of issues, "
            f"while LLM processing handled {metrics.llm_fixed_percentage:.1%} of remaining problems. "
        ])
        
        if metrics.quarantined_rows > 0:
            lines.append(
                f"{metrics.quarantined_rows:,} rows were quarantined for manual review. "
        ...
        lines.extend([
            ...
            "- `errors.csv` - Quarantined rows with error details" if metrics.quarantined_rows > 0 else "",
